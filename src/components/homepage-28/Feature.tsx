...
const FEATURES = [
  {
    ...
    image: {
      ...
      wrapperClass: 'max-w-[371px] max-h-[174px] w-full',
      figureClass: '',
    },
    points: [
  ...
  },
  {
    ...
    image: {
      ...
      wrapperClass: 'max-w-[344px] max-h-[174px] w-full overflow-hidden',
      figureClass: '',
    },
    points: [
    ...
    extraCardPadding: 'lg:py-[42px] md:py-[50px]',
  },
  {
    id: 3,
    ...
    image: {
      ...
      wrapperClass: 'max-w-[371px] overflow-hidden w-full rounded-[10px]',
      figureClass: 'rounded-[10px]',
    },
    points: [
          ...
          <div className="space-y-5 max-w-[724px] md:w-full mx-auto text-center">
            ...
            <div className="space-y-3">
              <RevealAnimation delay={0.2}>
                <h2>Key features of Fascinante Digital CRM</h2>
              </RevealAnimation>

              <RevealAnimation delay={0.3}>
                <p className="text-secondar dark:text-accent">
                  Fascinante Digital CRM is designed to simplify and supercharge your customer relationship management. With a
          ...
          <div className="grid grid-cols-12 items-center justify-center gap-y-8 md:gap-y-0 md:gap-x-8">
            {FEATURES.map((f, idx) => (
              <RevealAnimation key={f.id} delay={0.4 + idx * 0.1}>
                <div
                  className={`max-w-[408px] max-h-[400px] h-full col-span-12 md:col-span-6 lg:col-span-4 sm:w-full px-8 py-[42px] rounded-[20px] bg-background-2 dark:bg-background-5 space-y-6 ${
                    f.extraCardPadding ?? ''
                  }`}>
                  {/* Image */}
                  <figure className={f.image.wrapperClass}>
                    {/* Light */}
                    <Image
                      src={f.image.light}
                      alt={f.image.alt}
                      width={371}
                      height={174}
                      className={`size-full object-cover dark:hidden ${f.image.figureClass || ''}`}
                    />
                    {/* Dark */}
                    <Image
                      src={f.image.dark}
                      ...
                      width={371}
                      height={174}
                      className={`size-full object-cover hidden dark:inline-block ${f.image.figureClass || ''}`}
                    />
                  </figure>
